...
func testSingle(log *zap.Logger, data []Entry) {
	log.Sugar().Info("Trying connect redis...")
	rdb := redis.NewClient(&redis.Options{Addr: "localhost:6379"})
	ctx := context.Background()
	if err := rdb.Ping(ctx).Err(); err != nil {
		log.Sugar().Error("Cannot connect redis")
		return
	}
	...
	log.Sugar().Info("Successfully connected to redis")
